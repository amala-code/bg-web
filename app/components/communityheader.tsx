...
const CommunityHero = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-cyan-50 to-blue-50 overflow-hidden">
      {/* Hero Section */}
      <section className="relative px-4 sm:px-6 py-12 sm:py-20 lg:py-32 noise-bg overflow-hidden">
        {/* Decorative floating elements */}
        <div className="absolute top-10 sm:top-20 right-10 sm:right-20 w-32 sm:w-72 h-32 sm:h-72 bg-gradient-to-br from-cyan-300/20 to-blue-400/20 rounded-full blur-3xl float-element" />
        <div className="absolute bottom-10 sm:bottom-20 left-10 sm:left-20 w-48 sm:w-96 h-48 sm:h-96 bg-gradient-to-br from-teal-300/20 to-cyan-400/20 rounded-full blur-3xl float-element" style={{ animationDelay: '1s' }} />
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="grid lg:grid-cols-2 gap-8 sm:gap-16 items-center">
            ...
            >
              <h1 className="playfair text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-black text-slate-900 mb-4 sm:mb-6 leading-tight">
                The Only
                <span className="block bg-gradient-to-r from-cyan-600 via-blue-600 to-indigo-600 bg-clip-text text-transparent">
                  Community of
                </span>
                <span className="block">Creators & Pros</span>
                <span className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl">You Will Ever Need</span>
              </h1>
              ...
              <motion.button
                whileHover={{ scale: 1.05, boxShadow: "0 20px 40px rgba(6, 182, 212, 0.3)" }}
                whileTap={{ scale: 0.98 }}
                className="group relative px-6 sm:px-10 py-4 sm:py-5 bg-gradient-to-r from-cyan-500 via-blue-500 to-blue-600 text-white text-base sm:text-lg font-bold rounded-2xl shadow-2xl overflow-hidden mx-auto lg:mx-0"
              >
            ...
            {/* Right Content - Profile Cards - Desktop */}
            <motion.div
              initial={{ opacity: 0, scale: 0.8 }}
              animate={{ opacity: 1, scale: 1 }}
              transition={{ duration: 0.8, delay: 0.3 }}
              className="relative h-[500px] mt-8 lg:mt-0 hidden lg:block"
            >
              {communityMembers.map((member, index) => {
                let top, left, transform;
                if (index === 0) { top = '50%'; left = '20%'; transform = 'translateY(-50%)'; }
            ...
            {/* Right Content - Profile Cards - Mobile/Tablet */}
            <motion.div
              initial={{ opacity: 0, scale: 0.8 }}
              animate={{ opacity: 1, scale: 1 }}
              transition={{ duration: 0.8, delay: 0.3 }}
              ...
              <div className="flex justify-center items-center gap-4 flex-wrap">
                {communityMembers.map((member, index) => (
                  <motion.div
                    ...
                    className={index === 0 ? 'w-full flex justify-center' : ''}
                  >
                    <motion.div
                      whileHover={{ scale: 1.05 }}
                      className={`relative bg-gradient-to-br ${member.color} p-1 rounded-2xl shadow-xl cursor-pointer`}
                      style={{ 
                        width: index === 0 ? '200px' : '140px',
                        height: index === 0 ? '240px' : '180px'
                      }}
                    >
                    ...
                    {index === 0 && (
                      <motion.div
                        ...
                        animate={{ opacity: 1, y: 0 }}
                        transition={{ delay: 1 }}
                        className="absolute -bottom-4 left-1/2 transform -translate-x-1/2 bg-white px-4 py-2 rounded-full shadow-lg border-2 border-cyan-100 whitespace-nowrap"
                      >
                        <div className="flex items-center gap-2">
                          <div className="w-2 h-2 bg-gradient-to-r from-cyan-500 to-blue-500 rounded-full animate-pulse" />
                          <span className="font-semibold text-slate-700 text-sm">Network and Grow</span>
                        </div>
                      </motion.div>
                    )}
                  </motion.div>
      ...
      {/* Benefits Section */}
      <section id="benefits-section" className="px-4 sm:px-6 py-12 sm:py-20 lg:py-32 bg-white/50 backdrop-blur-sm relative noise-bg">
        <div className="max-w-7xl mx-auto">
          <div className="grid lg:grid-cols-2 gap-8 sm:gap-16 items-center">
            {/* Profile Card */}
            <motion.div
            ...
            >
              <div className="relative inline-block">
                ...
                <motion.div
                  ...
                  viewport={{ once: true }}
                  transition={{ delay: 0.5, type: "spring" }}
                  className="absolute -top-4 sm:-top-6 -right-4 sm:-right-6 bg-gradient-to-br from-teal-400 to-cyan-500 w-14 h-14 sm:w-20 sm:h-20 rounded-xl sm:rounded-2xl flex items-center justify-center text-2xl sm:text-3xl shadow-xl transform rotate-12"
                >
                  💼
                </motion.div>
                ...
                <motion.div
                  ...
                  viewport={{ once: true }}
                  transition={{ delay: 0.7, type: "spring" }}
                  className="absolute -bottom-3 sm:-bottom-4 -left-3 sm:-left-4 bg-white px-4 py-2 sm:px-6 sm:py-3 rounded-xl sm:rounded-2xl shadow-xl border-2 border-blue-100"
                >
                  <p className="text-xs sm:text-sm font-semibold text-slate-600">Anna</p>
                  <p className="text-[10px] sm:text-xs text-slate-400">Hey, let's connect! 👋</p>
                </motion.div>
              </div>
            </motion.div>
            ...
            >
              <motion.h2
                variants={itemVariants}
                className="playfair text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-black text-slate-900 mb-3 sm:mb-4 text-center lg:text-left"
              >
                One Community,
                <span className="block bg-gradient-to-r from-cyan-600 to-blue-600 bg-clip-text text-transparent">
                  Tens Of Benefits
              ...
              <motion.p
                variants={itemVariants}
                className="text-base sm:text-lg text-slate-600 mb-8 sm:mb-12 text-center lg:text-left"
              >
                Everything you need to succeed in one powerful platform
              </motion.p>
              ...
              <div className="space-y-4 sm:space-y-6">
                {benefits.map((benefit) => (
                  ...
                  >
                    <div className="flex-shrink-0 w-12 h-12 sm:w-16 sm:h-16 bg-gradient-to-br from-cyan-500 to-blue-600 rounded-xl sm:rounded-2xl flex items-center justify-center text-xl sm:text-2xl shadow-lg group-hover:shadow-2xl transition-shadow">
                      {benefit.icon}
                    </div>
                    <div>
                      <h3 className="text-lg sm:text-xl font-bold text-slate-900 mb-1 sm:mb-2">{benefit.title}</h3>
      ...
      {/* Target Audience Section */}
      <section className="px-4 sm:px-6 py-12 sm:py-20 lg:py-32 noise-bg relative overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-b from-transparent via-cyan-50/30 to-transparent" />
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          ...
          >
            <h2 className="playfair text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-black text-slate-900 mb-4 sm:mb-6">
              Who Is It For?
            </h2>
          ...
          >
            {targetAudience.map((audience, index) => (
              <motion.div
                ...
                className="group relative"
              >
                <div className={`relative bg-gradient-to-br ${audience.gradient} p-1 rounded-2xl sm:rounded-3xl shadow-xl transition-all duration-300 ${hoveredPerson === index ? 'shadow-2xl' : ''}`}>
                  <div className="bg-white rounded-2xl sm:rounded-3xl p-6 sm:p-8 h-full">
                    {/* Avatar circle */}
                    <div className={`w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 mx-auto mb-4 sm:mb-6 bg-gradient-to-br ${audience.gradient} rounded-full flex items-center justify-center text-2xl sm:text-3xl md:text-4xl shadow-lg group-hover:scale-110 transition-transform`}>
                      {audience.icon}
                    </div>
                ...
                {hoveredPerson === index && (
                  <motion.div
                    initial={{ opacity: 0, scale: 0 }}
                    animate={{ opacity: 1, scale: 1 }}
                    className="absolute -top-2 -right-2 w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-br from-cyan-400 to-blue-500 rounded-xl flex items-center justify-center text-lg sm:text-xl shadow-lg z-20"
                  >
                    ✨
                  </motion.div>
                )}
